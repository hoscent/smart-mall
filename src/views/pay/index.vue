<template>
  <div class="pay">
    ...
    <div class="footer-fixed">
      ...
      <div class="tipsbtn" @click="payOrder">提交订单</div>
    </div>
  </div>
</template>
...
import { checkOrder, checkSubmit } from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    getOrderList () {
      if (this.mode === 'cart') {
        checkOrder(this.mode, { cartIds: this.cartIds }).then(res => {
          const data = res.data
...
.address {
  ...
  color: #666;
  position: relative;
  background: url(@/assets/img/border-line.png) bottom repeat-x;
