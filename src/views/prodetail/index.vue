<template>
  <div class="prodetail">
    ...
    <div class="info">
      <div class="title">
        <div class="price">
          <span class="now">￥{{ data.goods_price_min }}</span>
          <span class="oldprice">￥{{ data.goods_price_max }}</span>
        </div>
        <div class="sellcount">已售{{ data.goods_sales }}件</div>
      </div>
    ...
    <div class="footer">
      ...
      <div class="btn-buy" @click="buyThis">立刻购买</div>
      <van-action-sheet v-model="showPannel" :title="mode === 'cart' ? '加入购物车' : '立刻购买'">
        <div class="product">
          ...
          <div class="showbtn" v-if="data.stock_total > 0">
            <div class="btn" v-if="mode === 'cart'" @click="addCartClick">加入购物车</div>
...
import mallConfirm from '@/mixins/mallConfirm'
...
export default {
  ...
  methods: {
    ...
    getDetail () {
      getProDetail(this.goodsId).then(res => {
    ...
    getComment () {
      getProComments(this.goodsId, 3).then(res => {
        const data = res.data
    ...
    addCartClick () {
      if (!this.isLogin()) return
      addCart(this.goodsId, this.addCount, this.data.skuList[0].goods_sku_id).then(res => {
        const data = res.data
